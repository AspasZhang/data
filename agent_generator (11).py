...
class AgentGenerator:
        ...
        while True:
            ...
            if relevant_entities and len(relevant_entities) > 1 and should_batch_execute(reasoning, relevant_entities):
                ...
                for entity in relevant_entities:
                    ...
                    # 执行工具（传递CoT帮助World Model生成更准确的响应）
    ...
    def _generate_summary(self, question: str, all_steps: List[Dict], state: StateManager,
        ...
        # 构建prompt - 让LLM分析完整流程
        prompt = f"""你是一个网络故障诊断专家。请仔细分析以下完整的故障诊断流程，并生成诊断总结报告。
...
3. **输出格式**：
   - 如果**发现异常**：列出所有异常节点（设备/接口/IP），每个节点包含状态、原因、修复建议
   - 如果**确实没有异常**：只返回一个总体结论节点，明确说明"未发现异常"

4. **输出JSON数组**：
```json
[
  {{
    "节点名称": "设备XXX的接口YYY",  // 具体的设备/接口名
    "检测项": "接口状态检查",
    "状态": "异常",  // 或"正常"
    "原因": "具体的异常原因（基于observation中的实际数据）",
    "修复建议": "具体的修复建议"
  }},
  {{
    "节点名称": "总体结论",
...
只输出JSON数组，不要有其他文字。"""
        ...
        try:
            ...
            # 验证总结质量
            has_anomaly_in_summary = any(
                node.get('状态', '').lower() in ['异常', 'down', 'error', 'abnormal']
                for node in summary_nodes
                if '总体结论' not in node.get('节点名称', '')
            )
            
            # 如果有异常步骤但总结说没有，打印警告
            if anomaly_steps and not has_anomaly_in_summary:
                print(f"   ⚠️  警告：检测到异常步骤{anomaly_steps}，但总结未识别异常")
                print(f"   💡 总结内容：{summary_nodes}")
