...
class AgentGenerator:
    ...
    def generate(self, 
        ...
        world_model = EnhancedWorldModel(
            ...
            tool_manager=self.tool_manager  # 传入tool_manager以获取输出格式
        )
        ...
        if not has_anomaly:
            ...
            for tool in fallback_tools:
                if tool in self.tool_manager.tools:
                    selected_tool = tool
                    break
            
            if not selected_tool and self.tool_manager.tools:
                selected_tool = list(self.tool_manager.tools.keys())[0]
