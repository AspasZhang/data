...
class AgentGenerator:
    ...
    def _generate_summary(self, question: str, all_steps: List[Dict], state: StateManager) -> tuple:
        ...
        # 构建详细的执行过程
        execution_details = []
        detected_entities = {}  # 记录检测到的实体及其状态
        ...
        for i, step_dict in enumerate(all_steps, 1):
            ...
            if step_key in step_dict:
                ...
                for action_obs in coa:
                    ...
                    execution_details.append(step_info)
                    
                    # 分析observation，提取检测节点和状态
                    self._extract_entity_status(observation, tool_name, detected_entities)
        
        # 构建prompt
        execution_summary = "\n".join([
            f"Step {item['step']}: {item['cot']}\n  工具: {item['tool']}\n  结果: {str(item['observation'])[:200]}..."
        ...
        if detected_entities:
            entities_info = "\n【检测到的节点】\n"
            for entity, info in detected_entities.items():
                entities_info += f"- {entity}: {info}\n"
        
        prompt = f"""你是一个网络故障诊断专家。请基于以下诊断过程，生成一份结构化的总结分析报告。
...
请生成一个JSON数组格式的总结报告，数组中每个元素代表一个检测节点（如设备、接口等）。

每个检测节点应包含：
- "节点名称": 设备名、接口名等
- "检测项": 检测的具体内容（如接口状态、错包统计等）
- "状态": "正常" 或 "异常" 或 "未知"
- "原因": 如果异常，说明原因；如果正常，说明"无异常"
- "修复建议": 如果异常，提供修复建议；如果正常，说明"无需处理"
...
只输出JSON数组，不要有其他文字。"""
        ...
        try:
            ...
            # 转换为coa格式（多个action-observation对）
            summary_coa = []
    ...
    def _extract_entity_status(self, observation: Any, tool_name: str, detected_entities: Dict):
        """
        从observation中提取实体及其状态
        
        Args:
            observation: 工具返回的观察结果
            tool_name: 工具名称
            detected_entities: 用于存储检测到的实体信息
        """
        if isinstance(observation, dict):
            # 提取接口信息
            if '接口' in observation or 'interface' in str(observation).lower():
                interface = observation.get('接口') or observation.get('interface')
                ...
                if interface:
                    ...
                    if status:
                        detected_entities[key]['状态'] = status
                    
                    # 记录其他关键信息
                    for field in ['IP地址/掩码', '速率', '错包', 'CRC错包']:
                        if field in observation:
                            detected_entities[key][field] = observation[field]
            
            # 提取设备信息
            if '设备' in observation or 'device' in str(observation).lower():
                device = observation.get('设备') or observation.get('device')
                if device:
                    key = f"设备_{device}"
                    if key not in detected_entities:
                        detected_entities[key] = {}
                    detected_entities[key]['检测工具'] = tool_name
        
        elif isinstance(observation, list):
            # 处理列表结果
            for item in observation:
                if isinstance(item, dict):
                    self._extract_entity_status(item, tool_name, detected_entities)
