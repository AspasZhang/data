...
class AgentGenerator:
    ...
    def _generate_summary(self, question: str, all_steps: List[Dict], state: StateManager) -> tuple:
        ...
        # 构建诊断过程摘要
        execution_summary = f"诊断过程共{len(all_steps)}步，检测了{len(all_entities)}个节点"
        
        # 构建发现摘要
        findings_summary = ""
        if findings.get('anomalies'):
            findings_summary += f"\n发现{len(findings['anomalies'])}个异常："
            for anomaly in findings['anomalies'][:3]:  # 最多列3个
                findings_summary += f"\n  - {anomaly}"
