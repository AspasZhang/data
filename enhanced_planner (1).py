...
6. **重要：tool_request中的参数必须使用【实体信息】中提供的设备名、接口名等，确保参数准确**
...
{{
  ...
  "tool_request": {{
    "device_name": "{entities.get('device', '设备名')}",
    "interface_name": "{entities.get('interface', '接口名')}",
    "其他参数": "根据工具需要填写"
  }},
...
- tool_request中的device_name和interface_name必须使用【实体信息】中的值
