...
2. **参数准确**：严格按照工具的参数定义填写tool_request，必需参数（标记✓）必须提供
3. **连贯性**：说明这一步如何承接上一步的发现，期望验证或排除什么
4. **使用准确参数**：从【相关参数】中获取device_name、interface_name等信息
...
{{
  ...
  "tool_request": {{
    "参数名1": "参数值1（根据工具参数定义填写）",
    "参数名2": "参数值2"
  }},
  "reasoning": "选择此工具的原因：基于上一步的XXX发现，需要验证/排除YYY，因此选择此工具",
...
- tool_request的参数名必须与工具定义中的参数名完全一致
"""
        return prompt
