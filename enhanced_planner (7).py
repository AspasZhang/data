...
class EnhancedPlanner:
    ...
    def select_next_tool(self, state: StateManager, goal: Dict[str, Any], 
        ...
        # 最后的备选方案：选择一个默认工具
        print(f"  ⚠️  选择备用工具...")
        return self._select_fallback_tool(state, goal)
...
**⚠️ 重要：tool_name必须从上述列表中精确选择，不要添加、删除或修改任何字符！**
...
**关键要求：**
1. **思考过程**：描述这一步的诊断思路和要做什么，不需要解释为什么选择这个工具
...
{{
  ...
  "reasoning": "描述这一步的思考过程：基于上一步的XXX发现，现在需要检查/确认YYY。如果是批量操作，说明'逐一检查所有XXX'",
...
注意：
- reasoning描述这一步要做什么和思考过程，不要写"选择XX工具的原因"
