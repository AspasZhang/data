...
"""
        ...
        if any(all_known_values.values()):
            ...
            if all_known_values['device_name']:
                param_lines.append(f"  可用设备名: {', '.join(all_known_values['device_name'][:5])}")
            if all_known_values['interface_name']:
                param_lines.append(f"  可用接口名: {', '.join(all_known_values['interface_name'][:10])}")
            if all_known_values['ip']:
                param_lines.append(f"  可用IP地址: {', '.join(all_known_values['ip'][:5])}")
            
            if param_lines:
                available_params_section = f"""
【可用参数值】（填写tool_request时必须从这里选择）
{chr(10).join(param_lines)}

⚠️ 关键：tool_request的参数值必须从上述列表中精确复制，不要使用"设备"、"接口"等描述性文字！
"""
...
2. **参数必须是具体值**：
   - ✅ 正确：device_name: "aggrleaf02_2_20.45"（具体的设备名）
   - ✅ 正确：interface_name: "40GE2/2/5"（具体的接口名）
   - ❌ 错误：device_name: "设备"（描述性文字）
   - ❌ 错误：device_name: "某个IP所在设备"（不存在的值）
   - ❌ 错误：interface_name: "接口"（占位符）
   - **参数来源只能是：①原问题中提供的；②前面步骤observation中获得的**
   - **从【相关参数】和【已知实体列表】中精确复制参数值，不要修改、翻译或描述**
