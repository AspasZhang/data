...
class EnhancedWorldModel:
    ...
    def _build_generation_prompt(self, tool_name: str, tool_request: Dict,
        ...
        # 格式约束
        if output_format:
            # 构建类型提示，enum 字段展示可选值而非类型名
            type_hints_parts = []
            for field, ftype in field_types.items():
                if field in enum_fields:
                    opts = "/".join(enum_fields[field])
                    type_hints_parts.append(f'"{field}": 从[{opts}]中选一个')
                else:
                    type_hints_parts.append(f'"{field}": {ftype}类型')
            type_hints = ", ".join(type_hints_parts)

            allowed_keys = ", ".join(f'"{k}"' for k in field_types)
            constraints.append(f"【格式】只输出以下字段：{{{allowed_keys}}}")
            constraints.append(f"【类型】{type_hints}。int类型不加引号，str类型加引号")
            constraints.append("【禁止】不要添加格式之外的任何字段，包括 type、enum 等隐性字段")

            # 为每个 enum 字段单独加强约束，并告知当前状态下的偏向选项
            for field, options in enum_fields.items():
                options_listed = "、".join(f'"{o}"' for o in options)
                constraints.append(
                    f'【枚举】"{field}"字段只能填 [{options_listed}] 中的一个值，'
                    f'"enum"禁止作为字段名出现在JSON中'
                )
