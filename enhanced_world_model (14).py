...
class EnhancedWorldModel:
    ...
    def _parse_output_format(self, output_format: str) -> Dict[str, str]:
        """
        解析Output格式，返回 {字段名: 类型串}。
        隐性字段 type / enum 直接过滤。
        enum 字段保留完整类型串，如 "enum[up,down,error]"。
        """
        if not output_format:
            return {}
        import re
        try:
            matches = re.findall(r'"([^"]+)":\s*"([^"]+)"', output_format)
            return {
                field: ftype.strip()
                for field, ftype in matches
                if field.lower() not in ('type', 'enum')
            }
        except:
            return {}

    def _parse_enum_options(self, type_str: str):
        """从类型串提取 enum 选项列表，非 enum 字段返回 None。"""
        if not type_str or not type_str.lower().startswith('enum'):
            return None
        import re
        bm = re.search(r'[\[(]([^\]\)]+)[\])]', type_str)
        if bm:
            raw = bm.group(1)
        elif ':' in type_str:
            raw = type_str.split(':', 1)[1]
        else:
            raw = type_str[4:]
        opts = [o.strip().strip(""""'""") for o in re.split(r'[,|]', raw) if o.strip()]
        return opts if opts else None

    def _enforce_output_schema(self, response: Dict[str, Any],
                                field_types: Dict[str, str]) -> Dict[str, Any]:
        """
        LLM 生成后强制回填：
        - 只保留 field_types 定义的字段，丢弃其余所有 key
        - enum 字段：值合法则用，否则取第一个选项（不语义匹配，不自编）
        - int 字段：强制转型，失败补 0
        - str 字段：转 str，缺失补空串
        """
        result = {}
        for field, type_str in field_types.items():
            ...
            enum_opts = self._parse_enum_options(type_str)

            if enum_opts is not None:
                if isinstance(raw, str) and raw in enum_opts:
                    result[field] = raw
                else:
                    if raw is not None and raw not in enum_opts:
                        print(f'      enum修正 [{field}]: "{raw}" not in {enum_opts}, use "{enum_opts[0]}"')
                    result[field] = enum_opts[0]

            elif type_str.strip() == 'int':
                ...
                result[field] = str(raw) if raw is not None else ''
    ...
    def _generate_tool_response(self, tool_name: str, tool_request: Dict, 
        ...
        try:
            ...
            # 强制按 output_format schema 回填，丢弃多余字段，修正 enum
            if output_format:
                _ft = self._parse_output_format(output_format)
                if _ft:
                    tool_response = self._enforce_output_schema(tool_response, _ft)
    ...
    def _build_generation_prompt(self, tool_name: str, tool_request: Dict,
        ...
        # 为 enum 字段追加选项约束提示
        for _ef, _et in field_types.items():
