...
class EnhancedWorldModel:
    ...
    def _generate_tool_response(self, tool_name: str, tool_request: Dict, 
        ...
        """
        # 构建prompt
        prompt = self._build_generation_prompt(tool_name, tool_request, variant, context, cot, question)
...
"""
        ...
        if self.tool_manager:
            output_format = self.tool_manager.get_tool_output(tool_name)
            if output_format:
                output_format_guidance = f"""
【输出格式要求】
此工具的标准输出格式：{output_format}
...
5. 数据要合理、具体、可信
6. **如果有诊断意图说明，确保返回的字段能够回答意图中的问题**
7. **如果有原始问题说明，响应数据必须与问题描述的故障类型一致**
8. **如果有输出格式要求，严格按照格式返回，包含所有必需字段**
...
- 如果诊断意图中提到要检查某些指标，务必在响应中包含这些字段
- **如果原始问题提到某种故障（如"接口down"），响应中必须反映这种故障**
"""
        return prompt
