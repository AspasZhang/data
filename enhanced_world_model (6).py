...
class EnhancedWorldModel:
    ...
    def _generate_tool_response(self, tool_name: str, tool_request: Dict, 
        ...
        """
        # 构建prompt
        prompt = self._build_generation_prompt(tool_name, tool_request, variant, context, cot, question, output_format)
    ...
    def _build_generation_prompt(self, tool_name: str, tool_request: Dict,
                                 variant: Dict, context: Dict, cot: str = None,
                                 question: str = None, output_format: str = None) -> str:
        """构建生成prompt"""
...
**关键要求：响应数据必须与原始问题保持一致！**
- 如果问题提到"接口状态Down"、"接口故障"，响应中接口状态应该是down/error
- 如果问题提到"CRC错包"、"丢包"，响应中应该包含相应的错包统计数据
- 如果问题提到"设备不可达"，响应中应该反映连接失败或设备异常
- **响应的异常类型必须与问题描述一致，不能矛盾**

例如：
- 问题："接口40GE2/2/5状态Down怎么办" → 响应中该接口状态必须是down
- 问题："为什么有大量CRC错包" → 响应中必须包含CRC错包数据
- 问题："设备连接失败" → 响应中必须反映连接异常
"""
...
**重要提示：**
- 仔细阅读上述思考过程，识别规划器想要检查哪些内容
- 如果思考中提到要检查某些字段（如"CRC错包"、"丢包率"、"接口状态"等），请在响应中包含这些字段
- 响应的字段应该能够回答规划器思考中的问题
"""
...
【关键约束 - 必须严格遵守】
1. **严格遵循Output格式**：如果有输出格式要求，必须完全按照格式定义的字段返回
2. **禁止使用"未提供"、"N/A"、"无"等占位符**：
   - 如果请求参数中有实体（如接口名、设备名），直接使用
   - 如果原始问题中提到实体，从问题中提取
   - 如果都没有，则编造一个合理的实体名称（如"40GE2/2/5"、"设备A"等）
   - 所有字段都必须有具体的值，不能是占位符
3. **数据必须具体可信**：
   - 接口名: 使用真实格式（如"40GE2/2/5"、"GigabitEthernet1/0/1"）
   - 数值: 使用合理范围（错包0-100000，速率10G/40G/100G等）
   - 状态: 根据variant要求返回具体状态（up/down/error等）
4. **与问题和上下文一致**：
   - 问题提到"接口Down" → 状态必须是down
   - 问题提到"CRC错包" → 必须包含CRC错包数据
   - 上下文有实体信息 → 直接使用上下文中的实体
...
注意：
- 第一次生成就要完全正确，严格遵循Output格式
- 所有字段值都必须是具体的、可信的数据
- 从请求参数、原始问题、上下文中提取实体，如果都没有就编造
