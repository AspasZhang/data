...
class ToolManager:
    ...
    def __init__(self, tools_file_path: str):
        ...
        self.tool_list: List[Dict[str, Any]] = []  # [{name: ..., description: ..., parameters: ..., output: ...}]
        self.tool_parameters: Dict[str, str] = {}  # {工具名: parameters字符串}
        self.tool_outputs: Dict[str, str] = {}  # {工具名: output格式字符串}
        self._load_tools()
        
        # 兼容性：让 available_tools 指向 tools
        # 这样旧代码使用 self.tool_manager.available_tools 也能正常工作
    ...
    def _load_tools(self):
        ...
        try:
            ...
            for line in lines:
                ...
                if line[0].isdigit() and '.' in line:
                    ...
                    if current_tool:
                        ...
                        if current_output:
                            tool_info['output'] = current_output
                            self.tool_outputs[current_tool] = current_output
                ...
                # 检查是否是Output行（直接存储为字符串）
                elif line.startswith('Output:'):
                    # 直接存储整行作为字符串
                    current_output = line.split('Output:', 1)[1].strip()
            ...
            tools_with_outputs = len(self.tool_outputs)
            print(f"✅ 成功加载 {len(self.tools)} 个工具 (其中 {tools_with_params} 个包含参数定义, {tools_with_outputs} 个包含输出格式)")
