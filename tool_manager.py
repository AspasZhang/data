...
class ToolManager:
    ...
    def __init__(self, tools_file_path: str):
        ...
        self.tool_parameters: Dict[str, Dict] = {}  # {工具名: parameters schema}
        self._load_tools()
        ...
        try:
            ...
            for line in lines:
                ...
                # 检查是否是Parameters行
                    ...
                    try:
                        params_json = line.split('Parameters:', 1)[1].strip()
                        current_parameters = json.loads(params_json)
                    except json.JSONDecodeError as e:
                        print(f"⚠️  解析 {current_tool} 的Parameters失败: {e}")
                        current_parameters = None
    ...
    def format_tool_parameters(self, tool_name: str) -> str:
        ...
        Returns:
            格式化的参数说明
        """
        params = self.get_tool_parameters(tool_name)
        if not params:
            return "  (无参数定义)"
        ...
        for param_name, param_info in properties.items():
            ...
            lines.append(f"  [{required_mark}] {param_name} ({param_type}): {param_title}{default_info}")
    ...
    def get_tools_with_parameters(self) -> str:
        ...
        for i, tool in enumerate(self.tool_list, 1):
            ...
            # 添加参数信息
            params_str = self.format_tool_parameters(tool['name'])
            if params_str and params_str != "  (无参数定义)":
                lines.append(f"   参数:")
